<template>
	<v-container fluid fill-height style="box-sizing:border-box; max-width:800px;">
		...
		<v-row no-gutters align="center">
			...
			<v-row no-gutters align="center" class="headline font-weight-bold py-5">
				팔로잉
				<v-chip class="ml-2 font-italic" color="orange" dark>beta</v-chip>
				...
				<v-list-item :href=" 'https://twitter.com/' + user.name " target="_blank">
					<v-row no-gutters align="center" justify="center" class="mt-5">
						...
						<v-row justify="center" align="center" style="max-width:700px">
							<v-col cols="auto">
								<v-avatar :tile="false" size="48" class="mr-1">
							...
							<template class="useritem large" v-else>
								<v-row justify="center">
									<v-card flat style="width:100%;  max-width:700px; background:transparent">
										<v-card-text class="grey--text text--darken-3 py-2">

											<v-row no-gutters>
												<v-col class="grow pr-2">
													<v-row no-gutters class="grey--text text--darken-1 pb-1">
														{{item.departedAt | datepassed}} · {{item.index.intent[0] | intent}}
													</v-row>
													<v-row no-gutters justify="start" style="overflow: hidden; line-height:1.6em;letter-spacing: 0.3px; text-overflow: ellipsis; max-height:100px;   display: -webkit-box;  -webkit-line-clamp:4; -webkit-box-orient:vertical;" :inner-html.prop="item.text | text | nonewline">

													</v-row>
												</v-col>
												<v-col cols="3" class="shrink" v-if="thumbImagesLocal(item)">
													<v-img :src="thumbImagesLocal(item)[0]" aspect-ratio="1.3" style="border-radious:5px" lazy-src="/_nuxt/assets/404.jpg" @error="item.attachment[0].src = '/_nuxt/assets/404.jpg'">
													</v-img>
												</v-col>
  ...
  async getfriends() {
    try {
      ...
      const res = await this.$axios.get(
        "/1.0/data/users/authenticated/friends/db"
      );
