...
import { Item, User } from './data/a.data.mongoose'
...
export default class AutoBot extends Tbot {
   ...
   */
  async promote(requestTweet: any, renew: boolean = false) {

    const bot = this.botConfig

    //리밋 체크
    let statuesCount = await Item.countDocuments({ "index.cat": bot.cat, "index.lang": bot.lang, departedAt: { "$gte": Date.now() - 1000 * 60 * 60 * 3 } })
    console.log('statuesCount : ', statuesCount)
    if (statuesCount > 140) {
      throw 'post status limit';
    }

    let result: { promoted: boolean, error?: any, retweetID?: any, filter?: any, saved?: Database.item.Iitem } = { promoted: false }
    ...
    try {
      ...
      let conditions = { id: itemTweet.user.id_str },
        update = { id: itemTweet.user.id_str, profileName: itemTweet.user.name, name: itemTweet.user.screen_name, sns: 'Twitter', lastVisit: Date.now() },
  ...
  async analyseUserUsage(user: Database.user.User, itemTweet) {
    //전체 요청 성공 수랑 이 봇에 최근에 이 트윗을 홍보 성공한 기록..? 시간차..? 가져오기
    let recentSuccess = await Database.Req.findOne({ userid: user.id, itemid: itemTweet.id_str, 'target.botID': this.botConfig.id, 'target.retweetID': { $ne: null } }).sort({ updatedAt: -1 }); let diffHours = undefined;
    if (recentSuccess) {
  ...
  async defaultFilter(req: Database.req.IReq, user: Database.user.User, itemTweet, item: Database.item.Iitem): Promise<{ ok: boolean, index?: 'tooOften' | 'polite' | 'tooShort' | 'default' | 'end' }> {
    ...
    // 내용이 너무 짧고 아무것도 아님
    let text: string = itemTweet.text
  ...
  */
  ...
  async filter(filterLayers: Array<any>, requestTweet: Twit.Twitter.Status, user: Database.user.UserDoc, itemTweet: Twit.Twitter.Status, item: Database.item.ItemDoc) {
    let filterData = {
      ...
      blacklist: {
        find(query: { type: string, id: string }) {
  ...
  async setfilterLayers(layers: Array<any>) {
  ...
  async catchDeleteEvent(deleteEvent) {
    ...
    Item.findOneAndUpdate({ retweetid: id }, { activated: false }, function (err, res) {
      if (err) {
  ...
  async catchDmEvent(dmEvent: any[]) {
    let bot = this;
      ...
      switch (metadata) {
        ...
        case "abuse_misuse":
          ...
          await Database.Chat.find({ senderID: sender_id, targetID: this.id, 'content.reportedItemID': { $exists: true } }).sort({ 'createdAt': -1 }).limit(1).then(chat => {
            ...
            Database.Report.find({ target_item: reportDoc.target_item, reason: { $exists: true } }).then(reports => {  //find by Target tweet id
      ...
      if (dm.message_data.entities.urls.length > 0) {
        ...
        try {
          ...
          if (parsed.blackItem) {
            ...
            let content: Database.chat.content = {
  ...
  async analyzeReport(targetItemID: string, senderUserID: string) {
    ...
    // 이미 제재된 트윗인지 확인
    let blackItemQuery: Database.blacklist.Iblacklist = {
    ...
    if (Database.item.userTypeGuard(reportedItem._user)) {
      ...
      // 이미 제재된 유저인지 확인
      let blacklistUserQuery: Database.blacklist.Iblacklist = {
        itemid: itemOwner.id,
        type: 'user'
      }
  ...
  async createReportByTwitterItemID(reason: Database.report.reason, targetItemid: string, senderUserID: string) {
    ...
    if (alreadyBlacklisted) {
      ...
      await Database.Report.find({ target_itemid: rep.target_item, reason: { $exists: true } }).then(async reports => {  //find by Target tweet id
  ...
  */
   ...
   * @param report report
   */
  async activateReport(reports: Database.report.reportDoc[]) {
  ...
  async deactivateUser(userid) {
    ...
    for (let i of user._items) {
      function itemTypeguard(item): item is Database.item.ItemDoc {
        return item && item.id
      }
      if (itemTypeguard(i)) {
        this.unRetweet(i.id)
  ...
  */
  replytoActivatedDMreport(e: Database.report.reportDoc) {
  ...
  async renewItemByTweet(itemTweet: any, forceRenew: boolean = false) {
    let botS3 = this.s3
    if (!botS3) {
      d('no s3')
      botS3 = null
    ...
    //아이템
    let item = await Item.findOne({ id: itemTweet.id_str })
    let itemUpdate: Database.item.Iitem = {};
    if (item && !forceRenew) {
    ...
    //유저
    let user = await User.findOne({ id: itemTweet.user.id_str })
    if (!user) {
      user = new User()
    }
   ...
   */
  async removeItemByTweetID(tweetID: string) {
    return Item.findOneAndRemove({ id: tweetID }).then(async removedItem => {

      const user = await User.findById(removedItem._user);
...
export async function updateUserbyItemTweet(itemTweet) {
  //유저
  let user = await User.findOne({ id: itemTweet.user.id_str })
  if (!user) {
    user = new User()
...
export async function AutoBotFactory(params: Database.bot.Ibot) {
...
export async function catchWebHookEvent(twitterEvent) {
...
export function closeDatabaseconn() {
...
export async function parseIntent(text: string) {
  ...
  function i(origin: string, params: string[], count = 1) { //포함했는지 알아내는 함수
...
export async function getUserReqSucceedCount(user, bot) {

  const query: Database.req.IReq = { userid: user.id }
...
function isMedia(media: any): media is Database.item.mediaDoc {
  return media && (media.origin !== undefined)
}
//S3에 존재하는 데이터의 key인가 체크하는 함수 (S3데이터만 따로 삭제된 경우를 false로 체크) 존재하면 true 아니면 false
export async function isExistInAWSS3(mediakey: string, botS3: iParser.S3) {
  let awsS3 = botS3.s3
  var params = {
    Bucket: botS3.bucket,
    Key: mediakey
...
export function isQuotedTweet(tweetObject: Twit.Twitter.Status) {
