...
export default class Tbot {
    ...
    constructor(id: string, keys: Twit.Options) {
        ...
        if (process.env.TWITTER_NOPOST && id !== '858242295807696896') { // exclude test bot id
            ...
            //@ts-ignore
            this.Twitter.post = function (path: string, params: Twit.Params, callback: Twit.Callback) {
 ...
 */
    ...
    getTweet(tweetID: string): Promise<Twit.Twitter.Status> {
        ...
        let get = () => {
            return new Promise(function (resolve, reject) {
                ...
                Twitter.get('statuses/show/:id', { id: tweetID, tweet_mode: 'extended' }, function (error, tweet: Twit.Twitter.Status, response) {
                    if (error) {
                        reject(error);
                    }
                    else {
     ...
     */
    getTopTweet(target: Twit.Twitter.Status, threadOption: Bot.threadOption = { all: false, passableIds: [] }): Promise<Twit.Twitter.Status> {
        let self = this;
        let Twitter = this.Twitter;
        let curTweet = target;
    ...
    */
    ...
    getWelcomeMessage() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            Twitter.get('direct_messages/welcome_messages/list', function (err, res: any) {
                if (err) {
                    console.log(err)
                    reject(err)
                }
    ...
    getWelcomeMessageRule() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            Twitter.get('direct_messages/welcome_messages/rules/list', function (err, res: any) {
                if (err) {
                    console.log(err)
                    reject(err)
                }
 ...
 * @return {Promise<*>}
 */
async function retry(fn, retriesLeft = 5, interval = 1000, exponential = false): Promise<any> {
    try {
        const val = await fn();
        return val;
    } catch (error) {
        ...
        } else throw new Error(error + ' Max retries reached');
    }
}
