...
router.put('/update', function (req, res, next) {
    ...
    Item.findOneAndUpdate({ id: req.body.id }, payload).then(saved => {
        return res.json(saved);
    }).catch(err => {
        console.error(err);
        return res.status(err.statusCode || 500).send(err.message || 'server error');
...
router.post('/promote', function (req, res, next) {
    ...
    Item.findOneAndUpdate({ id: req.body.id }, { departedAt: Date.now(), activated: true }).then(saved => {
        return res.json(saved);
    }).catch(err => {
        console.error(err);
        return res.status(err.statusCode || 500).send(err.message || 'server error');
...
router.get('/all', function (req, res, next) {
    ...
    return Item.find({activated:true}).sort({ 'departedAt': -1 }).limit(count).populate('_user').then((entities) => {
        res.json(entities)
...
router.get('/list', function (req, res, next) {
    ...
    let intent: string = req.query.intent
    ...
    return Item.list(condition, count, skip).then((entities) => {
...
router.get('/:cat/:intent', function (req, res, next) {
    ...
    return Item.list(query, count, skip).then((entities) => {
        d(`entities starts with : ${entities.list[0]}`)
        d(`entities next : ${entities.next[0]}`)
        res.send(entities);
    }).catch(err => { console.log(err); res.status(400).send(err.message) });
...
router.get('/:id', function (req, res, next) {
    ...
    Item.findOne({ id: id }).populate('_user').then(item => {
        if (item)
            res.json(item);
        else
