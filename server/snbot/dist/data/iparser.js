...
__export(require("./store.media"));
var store_media_1 = require("./store.media");
exports.S3 = store_media_1.default;
 ...
 * @param {*} itemTweet
 */
async function parseItemByTweet(itemTweet, mediaS3) {
    itemTweet.text = itemTweet.text ? itemTweet.text : itemTweet.full_text;
    if (!itemTweet.text) {
        throw new Error('Empty Tweet content cannot be saved!');
    }
    let item = await model_item_1.default.findOneAndUpdate({ id: itemTweet.id_str }, {}, { upsert: true, new: true });
...
exports.parseItemByTweet = parseItemByTweet;
function storeMediaByMeta(meta, hasNoAttachment, mediaS3) {
    ...
    if (meta.image && mediaS3 && exports.config.saveLinkMedia) {
        return mediaS3.storeMediaByLink(meta.image, { thumb: exports.config.saveThumbImage, small: shouldSaveSmallImage });
    }
    else {
        return;
    }
}
exports.storeMediaByMeta = storeMediaByMeta;
/**
 *
 * @param itemTweet
 */
async function parseAttachment(itemTweet, mediaS3) {
    ...
    for (const m of itemTweet.extended_entities.media) {
        ...
        if (mediaS3 && exports.config.saveAttachMedia) {
 ...
 * @param {String} link
 */
async function parseLinkMeta(link) {
...
exports.parseLinkMeta = parseLinkMeta;
/**
 ...
 */
async function parseMetaTCardImage(link) {
    let width = 150;
    let jpegQuality = 60;
...
exports.parseMetaTCardImage = parseMetaTCardImage;
function jimpImg(imageSrc, width, jpegQuality) {
