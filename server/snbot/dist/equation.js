...
exports.operation = {
    ...
    '==': (a, b) => b === a,
    '!=': (a, b) => b !== a,
    'incl': function (included, includes) {
...
function getNestedObjectValue(key, obj) {
...
async function calculate(equation, ref, filterData) {
    let pgOperandQueue = [];
    ...
    for (const el of equation) {
        try {
            if (el instanceof Operand) {
                ...
                let res = await exports.operation['find'](val, key, list);
                ...
                let res = exports.operation[el](a, b);
                d(`a:${a}, b:${b}, RES : ${res}`);
                pgOperandQueue.push(res);
                d(`queue : ${JSON.stringify(pgOperandQueue)} `);
            }
...
function makeFind(list, key, value) {
